...
if(type == 0) set term pslatex size 5, 3; set output 'EnergyProfile9_0_1.tex' 
##the next line is to see what is going on in a pict.  Also useful to save for viewing in PDF format.
if(type == 1) set term png; set output 'EnergyProfile9_0_1.png'
...
if(type == 0) set title 'Energy Profile.'
if(type == 1) set title 'Energy Profile.'
set xlabel ''
set ylabel 'Energy'
...
set key 0,1 spacing 1.6


plot 'EnergySurface9noise_1_0.dat' using 6:5 every 1 with points #,\
#'EnergySurface9noise_1_0.dat' using 7:5 every 1 with points,\
#'EnergySurface9noise_1_0.dat' using 8:5 every 1 with points
